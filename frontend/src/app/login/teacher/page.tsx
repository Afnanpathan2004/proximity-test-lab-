...
export default function TeacherLoginPage() {
  ...
  const onDemo = async () => {
    ...
    const demoEmail = 'demo-teacher@example.local';
    const demoPass = 'demo12345';
    try {
      try {
        await api.post('/api/auth/register', { name: 'Demo Teacher', email: demoEmail, password: demoPass, role: 'teacher' });
      } catch (e: any) {
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-md rounded-xl border bg-white dark:bg-gray-950 p-6 shadow-sm">
        ...
        <div className="mt-3">
          <button onClick={onDemo} className="w-full px-4 py-2 rounded border hover:bg-gray-50 dark:hover:bg-gray-800 transition active:scale-[.98]" disabled={loading}>Use Demo Account</button>
        </div>
      </div>
    </div>
  );
